...
lambda1 = 1; 
s=1./lambda1;   % Tasa de reclamaciones (gamma) 
...
Y = exprnd(lambda1, [1, N]);
X = gamrnd(1,1./lambda2, [1, N]);
...
u_1 = -(c./(c.*lambda2-lambda1)).*log(b.^(T.^(alpha)))-50;
u_2 = -(c ./ (c * lambda1 - lambda2)) .* log(b.^(T.^(alpha+0.1)))-50;
u_3 = -(c ./ (c * lambda1 - lambda2)) .* log(b.^(T.^(alpha+0.2)))-50;
...
plot(T, u_1, '-r', 'DisplayName', 'u_1');
plot(T, u_2, '-g', 'DisplayName', 'u_2');
plot(T, u_3, '-k', 'DisplayName', 'u_3');

% Graficar el capital
plot(T, capital(1:length(T)), '-b', 'LineWidth', 1);  % Tomar cada dos valores para el capital
legend off;
plot(T, capital_max, '-m', 'LineWidth', 1);
legend off;
xlim([0 A]);
xlabel('Tiempo');
ylabel('');
